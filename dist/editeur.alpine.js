    ...
    return {
      ...
      deleteColumn: () => ({ state, dispatch }) => {
        ...
        return deleteColumn(
          ...
          dispatch && ((tr2) => {
            if (table) {
              keepCursorInTable(tr2, table.pos);
            }
            dispatch(tr2);
          })
        );
...
// src/js/extensions/image-avec-largeur.js
var ImageAvecLargeur = index_default4.extend({
  addAttributes() {
    return {
      ...this.parent(),
      width: {
  ...
  addNodeView() {
    return ({ node, getPos, editor }) => {
      ...
      return {
        ...
        selectNode() {
          enveloppe.classList.add("est-selectionnee");
        },
        deselectNode() {
          enveloppe.classList.remove("est-selectionnee");
        }
      };
    };
  }
...
// src/js/extensions/table-cellule-avec-bordure.js
var TRAIT_BORDURE = "1px solid #000000";
var TableCellAvecBordure = TableCell.extend({
  addAttributes() {
    return {
      ...this.parent(),
      bordureTop: {
...
// src/js/tableau/appliquer-bordures.js
function appliquerBordures(editeur, cotes) {
  const { state } = editeur;
  const rect = (() => {
    try {
  ...
  if (!rect) return;
  const { map: map2, left, right, top, bottom, tableStart } = rect;
  let tr2 = state.tr;
  for (let ligne = top; ligne < bottom; ligne++) {
    for (let col = left; col < right; col++) {
      const posRelative = map2.map[ligne * map2.width + col];
      if (map2.map.indexOf(posRelative) !== ligne * map2.width + col) continue;
  ...
  editeur.view.dispatch(tr2);
}

// src/js/tableau/ruban-positionnement.js
function creerPositionneurRuban(ruban, conteneur) {
...
// src/js/tableau/figer-colonnes.js
function figerColonnesDuTableau(ed, table, tableStart) {
  const map2 = TableMap.get(table);
  let tr2 = ed.state.tr;
  let modifie = false;
  for (let col = 0; col < map2.width; col++) {
    const posRelative = map2.map[col];
    if (map2.map.indexOf(posRelative) !== col) continue;
    ...
    tr2 = tr2.setNodeMarkup(pos, void 0, { ...noeud.attrs, colwidth: largeurs });
    modifie = true;
  }
  if (modifie) ed.view.dispatch(tr2);
  return modifie;
}
function creerDetecteurApprocheRedim() {
...
// src/js/toolbar/dispatch-commandes.js
function attacherCommandes(barreOutils, editeur) {
  ...
  barreOutils.querySelectorAll("[data-commande]").forEach((bouton) => {
    bouton.addEventListener("click", () => {
      ...
      switch (bouton.dataset.commande) {
        ...
        case "augmenter-retrait":
          chain.sinkListItem("listItem").run();
          break;
        case "reduire-retrait":
        ...
        case "fusionner-cellules":
          chain.mergeCells().run();
          break;
        case "scinder-cellule":
          chain.splitCell().run();
          break;
...
// src/js/toolbar/grille-tableau.js
function attacherGrilleTableau(bouton, popup, editeur) {
...
// src/js/zoom/largeur-page.js
function creerGestionnaireLargeurPage({ conteneurEditeur, zonePage, facteurZoomConfort = 1.5 }) {
...
// src/js/core/editeur.js
var SrdTipTapEditor = class {
  constructor(config) {
  ...
  _construireEditeur() {
    ...
    this.editor = new Editor({
      ...
      extensions: [
        ...
        Color,
        LineHeight,
        index_default2.configure({ types: ["paragraph"] }),
        index_default3.configure({ multicolor: true }),
      ...
      this.editeur = new SrdTipTapEditor({
        ...
        telechargerImage: (fichier) => new Promise((resolve, reject) => {
          this.$wire.upload(
            ...
            fichier,
            () => {
              this.$wire.getFormComponentFileAttachmentUrl(statePath).then((url) => resolve({ url })).catch(reject);
            },
            () => reject(new Error("\xC9chec de l'envoi au serveur"))
          );
        }),
        controles: {
...
export {
  index_alpine_default as default
};
//# sourceMappingURL=editeur.alpine.js.map
